    ...
    private void HandleMessage(string message)
    {
        try
        {
            ...
            switch (type)
            {
                ...
                // ✅ NOVO: Tratamento de falha ao usar item
                case "itemUseFailed":
                ...
                case "error":
                    Debug.LogError("❌ Server error: " + json["message"]);
                    if (UIManager.Instance != null)
                    {
                        UIManager.Instance.AddCombatLog($"<color=red>Erro: {json["message"]}</color>");
                    }
                    break;
	...
	private void HandlePlayerAttack(JObject json)
{
    ...
    private void HandleStatusPointAdded(JObject json)
    {
        ...
        Debug.Log($"✅ Status point added: {data.stat} - Remaining points: {data.statusPoints}");
    ...
    private void HandleLootReceived(JObject json)
    {
        var data = new LootReceivedData
        {
            ...
            items = json["items"]?.ToObject<System.Collections.Generic.List<LootedItemData>>() ?? new System.Collections.Generic.List<LootedItemData>()
        };

        Debug.Log($"💰 Loot received: {data.gold} gold, {data.items.Count} items");
    ...
    private void HandleItemUseFailed(JObject json)
    {
        ...
        Debug.LogWarning($"⚠️ Item use failed: {reason} - {message}");
    ...
    private void HandleItemUnequipped(JObject json)
    {
        ...
        Debug.Log($"⚔️ Item unequipped from slot: {json["slot"]}");
